-- RED HUB (Fixed Hooks Connection)
pcall(function()
    ...
    local function safeLoadFromUrl(key, url)
        ...
        if ok2 then
            ...
            warn("❌ Loadstring failed for: " .. key)
        end
    end

    -- Hooks functions - ĐẢM BẢO TẤT CẢ HOOKS ĐƯỢC ĐỊNH NGHĨA
    local hooks = {}
    ...
    hooks.FlyV3 = function(enable)
        ...
        if enable then
            if handles.FlyV3 then 
                print("FlyV3 already running")
                return 
            end
            ...
            local function startFlying()
                ...
                if not character or not character:FindFirstChild("HumanoidRootPart") then 
                    warn("No character or HRP for Fly")
                    return 
                end
            ...
            handles.FlyV3 = function()
                ...
                print("✅ FlyV3 cleaned up")
            end
            print("✅ FlyV3 activated")
            
        else
            tryCleanup("FlyV3")
            print("✅ FlyV3 deactivated")
        end
    end
        ...
        if enable then
            ...
            local conn = RunService.Heartbeat:Connect(function()
                ...
                if char and char:FindFirstChild("HumanoidRootPart") then
                    ...
                    local speed = 50 -- Tăng speed lên cho rõ hiệu ứng
    ...
    hooks.WallWalk = function(enable)
        print("WallWalk: " .. tostring(enable))
        if enable then
    ...
    hooks.TeleportGUI = function(enable)
        print("TeleportGUI: " .. tostring(enable))
        if enable then
    ...
    hooks.KillAura = function(enable)
        print("KillAura: " .. tostring(enable))
        if enable then
    ...
    hooks.Fighting = function(enable)
        print("Fighting: " .. tostring(enable))
        if enable then
    ...
    Rayfield:Notify({
        Title = "Red Hub",
        Content = "Menu đã tải thành công! Chức năng đã được kết nối.",
        Duration = 6,
    })

    print("✅ minh Hub loaded với hooks đã kết nối!")
end)
