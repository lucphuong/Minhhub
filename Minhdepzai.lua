...
pcall(function()
    ...
    hooks.FlyV3 = function(enable)
        if enable then
            ...
            local function loadFlyScript()
                pcall(function()
                    -- Load Fly Script từ nguồn đáng tin cậy
                    local flySource = game:HttpGet("https://raw.githubusercontent.com/XNEOFF/FlyScriptV3/main/FlyScriptV3.txt", true)
                    if flySource then
                        local flyFunction = loadstring(flySource)
                        if flyFunction then
                            handles.FlyV3 = flyFunction()
                            print("✅ Fly V3 loaded successfully")
                        end
                    end
                end)
            end
            ...
            -- Thử load fly script
            local success, error = pcall(loadFlyScript)
            
            if not success then
                warn("Fly V3 load failed: " .. tostring(error))
                -- Fallback: Fly script đơn giản
                handles.FlyV3 = createSimpleFly()
                print("🔄 Using simple fly as fallback")
            end
    ...
    print("✅ Red Hub loaded (Fly V3 fixed, hooks preserved, pcall-protected, executor-safe).")
end)
